...
5. **交付** —— 产物在 `out/`:每图 `.svg`(矢量,可放大)/ `.png` / `.meta.json`(记账)/ `.check.txt`(验收详情),
   外加一份汇总好的 **`out/待确认.md`**。把图与那份清单一起交人类。

### 「待确认」必须落盘

每抽完一张图,agent 要把**拿不准、需要人拍板的东西**写进 `figs/<id>.待确认.md`
(真的没有,就写"无")。`build.py` 会把这些**机械汇总**成 `out/待确认.md` 交给人类。

**为什么不能只在对话里说**:验收脚本只能证明「图与契约一致」,证明不了「契约与事实一致」——
那份要人拍板的清单才是这个工具**唯一的人机接口**。它要是只活在 agent 的会话里,
会话一结束(或上下文一压缩)就没了,"人拍板"这一步就断了。所以它是**产物**,不是聊天记录。

汇总动作刻意交给 `build.py` 而不是 agent:agent 只需要写自己那一张图的小文件,
不必读改写一个大文件(那正是最容易覆盖丢失的操作)。
